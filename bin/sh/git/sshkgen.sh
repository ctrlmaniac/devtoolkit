...
echo "=== SSH Key Generation for GitHub ==="
echo
...
if [ ! -d "$ssh_dir" ]; then
    ...
    if ! mkdir -p "$ssh_dir"; then
        ...
        echo "Please check permissions or create it manually: 'mkdir -p ${ssh_dir} && chmod 700 ${ssh_dir}'"
        exit 1
    fi
    if ! chmod 700 "$ssh_dir"; then
        echo "Error: Could not set permissions for ${ssh_dir} to 700."
        echo "Please set them manually: 'chmod 700 ${ssh_dir}'"
        exit 1
    fi
    ...
    if ! chmod 700 "$ssh_dir"; then
        echo "Warning: Could not ensure permissions for ${ssh_dir} are 700."
...
if [ -f "${key_filepath}" ] || [ -f "${key_filepath}.pub" ]; then
    echo "Warning: Key file '${key_filepath}' or '${key_filepath}.pub' already exists."
...
if [[ "$add_to_agent" =~ ^[Yy]$ ]]; then
    if ! command -v ssh-add &> /dev/null; then
        ...
        if [ -z "$SSH_AUTH_SOCK" ] ; then
            ...
            echo "Then run: ssh-add ${key_filepath}"
        fi
        ...
        if ssh-add "${key_filepath}"; then
            ...
            echo "To start an agent and add the key manually in your current shell:"
            echo "  eval \"\$(ssh-agent -s)\""
            echo "  ssh-add ${key_filepath}"
        fi
    fi
fi
echo
