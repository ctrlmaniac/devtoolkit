...
is_safe_target() {
    ...
    for path in "${critical_paths[@]}"; do
        # Check for exact match or if it's a parent of a critical path
        if [[ "$abs_dir_to_check" == "$path" || "$abs_dir_to_check/" == "$path/"* || "$path/" == "$abs_dir_to_check/"* ]]; then
            echo "ERROR: Target directory '$abs_dir_to_check' appears to be a critical system path or a parent of one." >&2
            echo "This script is intended for user project directories (ideally within your home directory)." >&2
...
echo "--- Permission Adjustment Script ---"
...
echo
echo "WARNING: Ensure '$ABS_TARGET_DIR' is the correct project directory."
