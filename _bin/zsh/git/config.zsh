...
CURRENT_SCRIPT_DIR_LOGIN="$(cd "$(dirname "${(%):-%x}")" &>/dev/null && pwd)"
CHECK_INSTALL_SCRIPT_PATH_LOGIN="${CURRENT_SCRIPT_DIR_LOGIN}/../utils/check-install.sh"

# Function to manually check for a command if check-install.sh is not available_manual_check_command() {
...
echo
echo -e "\n✨ All done! Ready to rock your code in WSL! 🚀"
